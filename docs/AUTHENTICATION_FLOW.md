...
## SP-Initiated Login (Auto Login / "Sign in with SAML")

When a service provider (e.g., Grafana with `auto_login` enabled) starts the flow, the browser is
redirected to the IdP SSO URL with an HTTP-Redirect binding AuthnRequest:

```
GET {ApiGW}/sso?SAMLRequest=<deflated+base64>&RelayState=<state>
```

1. **Lambda validates the AuthnRequest**: decodes it (base64 + raw DEFLATE), extracts the request
   `ID`, `AssertionConsumerServiceURL` and optional `Issuer`, then rejects non-HTTPS ACS URLs or
   malformed requests.

2. **Redirect to login page**: The Lambda responds with `302` to the static login page
   (`LOGIN_PAGE_URL`) carrying the context in the URL fragment:
   `https://<cloudfront>/#sp_acs=...&in_response_to=...&relay_state=...`
   The page JavaScript stores this context (and clears the fragment).

3. **Normal authentication**: The user logs in and selects a role as usual.

4. **Response generation**: The portal POSTs to `/sso` including the SP context fields
   (`sp_acs`, `in_response_to`, `relay_state`). The Lambda enforces that the selected role's
   stored `acs_url` matches `sp_acs`, includes `InResponseTo` in the SubjectConfirmationData,
   sets `<saml:Audience>` from the role's `audience` field, and appends `RelayState` to the
   auto-submit form.
...
- **Audience Restriction**: Assertions restricted to the SP entity ID configured per role (`audience` field; defaults to `urn:amazon:webservices` for AWS)
- **SP-Initiated Safety**: AuthnRequests must target HTTPS ACS URLs; assertions are only dispatched to ACS URLs registered on the user's roles
...
Generates SAML response for selected role, or handles SP-initiated AuthnRequests.

**SP-initiated** (GET or POST with `SAMLRequest`): validates the request and redirects to the login page.

**IdP-initiated** (form-encoded body):
```
username=john.doe&role_arn=arn:aws:iam::123456789012:role/AdminRole
```

Optional SP context fields (added automatically by the portal for SP-initiated logins):
```
sp_acs=https://mystack.grafana.net/saml/acs
in_response_to=_request-id-from-authnrequest
relay_state=<opaque state>
```

**Response** (HTML):
Auto-submit form with SAMLResponse (and RelayState when present)
